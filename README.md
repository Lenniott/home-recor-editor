...
**New** clears the session (confirms if there are unsaved changes). **Import** adds recordings to the current session — the first file opens it, a second Import attaches the other synced lane (max two). A sidecar next to a single file imported into an empty session still restores if present. **Add recording** is the same attach-lane action with a single-file picker. **Save**/Cmd+S writes next to the recording the first time (`<name>.hre.json`) and to that same file afterward, but it will not overwrite a project file that already exists under that name unless this session is already bound to it — **Save as** picks a different location. **Import** will not attach a recording that is already a lane in the session. **Open** opens a `.hre.json` directly — if a recording has moved, you're prompted to locate it. Cancelling Locate leaves the current project in place and shows an error. Once a project has a save location, edits (including a finished transcript) autosave shortly after you stop making them; the footer shows Saving…/Saved/Unsaved changes. Writes are atomic, so an interrupted save never leaves a corrupt project file.
...
- Both marker actions use the same selection, Mark/Unmark, click-to-select, draggable edges, and undo workflow. A cut always spans both tracks and remains a pending marker until preview/export. Cut edges can also be dragged in the shared marker lane. **Export** marks (one or both lanes) drag from the same waveform edges as silence; they do not change preview sound; overlapping export marks stay separate. Paint order is silence under cut under export.
...
- **Edits** reviews overlap suggestions and existing markers. Audition includes one second of context on either side. **Cmd/Ctrl-click** a mark (timeline or list) adds it to the selection; **Cmd/Ctrl+A** with a lane or the marks list focused selects every mark. **Backspace**, **Delete**, or **Remove** deletes the selection as one undo step. **Change to cut** / **Change to silence** apply to every selected mark. Changing a silence to a cut expands it to every track (then overlapping cuts merge); changing a cut to silence keeps all lanes as a synced mute. Each type change is one undo step.
- **Original** plays the sources; **Preview edits** auditions all silence and cut markers (export marks are ignored in preview). Only **Export** renders those edits into new audio files.
- Export shows its render, mix, encode, and file-writing progress in the export dialog after you choose what to write.
- Separate WAVs share one rate and duration. **Combined merge** writes `{stem}-merge.wav` (equal-gain sum, same math as the old mix). Written files are stereo by default (a one-channel source is copied to L and R). Pass `channels: "mono"` to downmix to one channel. Sample-rate mismatch still refuses before any write. File → Export chooses **all** (full timeline) or **clips** (one range per export mark; disabled until an export mark exists), layout merge/separate/both, and stereo/mono. **Audio** and **Transcript** checkboxes write WAVs and/or transcript `.txt`. Full-timeline transcript is `{stem}-transcript.txt`; clips write `{stem}-clip-{n}.txt` beside each clip WAV. Each txt is speaker paragraphs (clock + name, then that run of words on one line) — not one word per line. **Apply edits** (on by default) drops cut and silenced words from the txt and remaps clocks for cuts. File → **Export transcript…** and **Export transcript…** on the transcript pane open the same dialog with only Transcript on (no stereo/mono, no separate/merge; one Export button). An existing txt path is refused, not overwritten. Clips + transcript keeps words that overlap that export mark's lanes. Export, silence, and cut marks all drag from their start/end edges; overlapping export marks stay separate records.
